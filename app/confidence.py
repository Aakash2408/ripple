...
def format_pr_body(change_description: str, source_repo: str,
    ...
    # These two rows previously asserted things Ripple had not measured:
    # "Co-change pattern detected in git history" when no git history was
    # scanned, and "Similar fixes merged without revert" when no prior fix had
    # been tracked. Both now describe what actually produced the score.
    obs_reason = reasons[0] if reasons else "Static reference match in consumer source"
    hist_reason = (
        "Prior for this fix type -- not a measurement of past merges in this repo"
    )
    ...
    body_parts.append(f"| Historical accuracy | {hist_score:.2f} | {hist_reason} |")
    ...
    # Footer
    # This previously read "PropBench v1 (882 entries) | Learning: enabled" on
    # EVERY PR. Both were false: propbench_data/ is not vendored into this repo
    # so no entry has ever been loaded, and no learning channel has run in
    # production (see .kiro/steering/ripple-invariants.md). The footer now
    # states provenance only -- claims about learning belong here only once a
    # learning channel actually reports having run.
    body_parts.extend([
        "---",
        f"*Generated by [Ripple](https://ripple-cnn.pages.dev) | "
        f"Detection: {' + '.join(sources[:3])} | Fix: {fix_label}*",
    ])
