...
#: A pattern this dormant leaves retrieval. It is NOT deleted -- see `archived`.
#:
#: DERIVED, not chosen. Measured with tools/measure_store_constants.py against
#: PropBench's 871 dated entries mined from real OSS repos (0 undated):
#:
#:     distinct files touched by a breaking change   4097
#:     files touched on more than one date            488   (11.9%)
#:     recurrence intervals measured                  764
#:     median  10d    p75  30d    p90 141d    p95 260d    max 2837d
#:
#: 141 is the p90: it keeps a pattern retrievable across 90% of the intervals at
#: which the same file actually comes back. The previous value of 90 sat around
#: p85 and was picked because it is a round number.
#:
#: TWO HONEST LIMITS ON THAT NUMBER.
#:
#: It is a PROXY. It measures "how long until a change of this shape returns",
#: which is what decides whether dormancy ever bites. It does NOT measure "how
#: long until a fix strategy goes stale against a refactored codebase", which is
#: the quantity we actually want and which needs merged-PR outcomes Ripple does
#: not yet have.
#:
#: And the threshold is NOT load-bearing, which the same measurement shows: 88%
#: of files are touched by a breaking change exactly once, so for the dominant
#: case no threshold changes anything. Both error directions are also cheap --
#: too short costs one fix generated by the template instead of by the pattern
#: (and dormancy is reversible, so a clean merge revives it), while too long
#: costs a stale strategy that still cannot reach AUTO without compiling. Do not
#: spend effort tuning this until a real store exists.
ARCHIVE_AFTER_DAYS = 141

#: Ceiling on ACTIVE patterns. Eviction archives rather than drops.
#:
#: DERIVED. save() rewrites the whole file on EVERY recorded outcome and load()
#: runs at module import, so both were measured as N grows (448 bytes/row):
#:
#:         N     file     save     load   retrieve
#:      2000    875kB   20.3ms   12.2ms     2.2ms
#:      5000   2188kB   50.1ms   31.2ms     5.5ms
#:     10000   4375kB  101.8ms   63.8ms    11.0ms
#:     25000  10938kB  261.6ms  160.4ms    27.4ms
#:
#: The finding is that COST DOES NOT BIND anywhere near these sizes. Against a
#: 10s GitHub webhook budget, 262ms of bookkeeping at 25k rows is immaterial,
#: and retrieval stays under 30ms. So the earlier value of 2000 was not
#: protecting anything measurable -- it was a round number, and a conservative
#: one that would have started archiving real evidence early for no reason.
#:
#: The cap therefore exists to bound GROWTH on the mounted volume, not to
#: protect latency, and it is set to 5000 to match pr_ledger._MAX_ROWS -- one
#: number to reason about across both persisted stores rather than two.
MAX_ACTIVE_PATTERNS = 5000
